    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ...
          ppw->pv->y[ppw->pv->index_pt_delta_fld] = (1+w_fld-pba->delta_DMDE*2.)
                                                    *(1+w_fld+pba->delta_DMDE/3.)
                                                    /(12*w_fld*w_fld-2*w_fld-3*w_fld*pba->delta_DMDE+7*pba->delta_DMDE-14)
                                                    *(-2.*ppw->pv->y[ppw->pv->index_pt_delta_g]/(1+1./3.));
          ppw->pv->y[ppw->pv->index_pt_theta_fld] = k*tau*(1+w_fld+pba->delta_DMDE/3.)
                                                    /(12*w_fld*w_fld-2*w_fld-3*w_fld*pba->delta_DMDE+7*pba->delta_DMDE-14)
                                                    *(-2.*ppw->pv->y[ppw->pv->index_pt_delta_g]/(1+1./3.));
        }
  ...
  if (_scalars_) {
    ...
    if (pba->has_cdm == _TRUE_) {
      ...
      if (ppt->gauge == synchronous) {
        class_call(background_w_fld(pba,a,&w_fld,&dw_over_da_fld,&integral_fld), pba->error_message, ppt->error_message);
        double v_T = 0.0;
        double sum_rho_plus_p_theta = 0.0;
        double sum_rho_plus_p = 0.0;

        sum_rho_plus_p_theta += pvecback[pba->index_bg_rho_b] * y[pv->index_pt_theta_b];
        sum_rho_plus_p += pvecback[pba->index_bg_rho_b];
      
        sum_rho_plus_p_theta += (4./3.) * pvecback[pba->index_bg_rho_g] * y[pv->index_pt_theta_g];
        sum_rho_plus_p += (4./3.) * pvecback[pba->index_bg_rho_g];

        if (pba->has_ur == _TRUE_) {
          sum_rho_plus_p_theta += (4./3.) * pvecback[pba->index_bg_rho_ur] * y[pv->index_pt_theta_ur];
          sum_rho_plus_p += (4./3.) * pvecback[pba->index_bg_rho_ur];
        }
 
/*         sum_rho_plus_p_theta += pvecback[pba->index_bg_rho_fld] * (1 + w_fld) * y[pv->index_pt_theta_fld];
        sum_rho_plus_p += pvecback[pba->index_bg_rho_fld] * (1 + w_fld); */

        v_T = sum_rho_plus_p_theta / (k * sum_rho_plus_p);

        dy[pv->index_pt_delta_cdm] = 0.0
                                  -metric_continuity
                                  +pba->delta_DMDE*pvecback[pba->index_bg_rho_fld]/pvecback[pba->index_bg_rho_cdm]*(
...
/*         sum_rho_plus_p_theta += pvecback[pba->index_bg_rho_fld] * (1 + w_fld) * y[pv->index_pt_theta_fld];
        ...
        dy[pv->index_pt_delta_fld] = -(1+w_fld)*(y[pv->index_pt_theta_fld]+metric_continuity)
                                         -3.*a_prime_over_a*(1.-w_fld)*(
                                          ...
                                          -pba->delta_DMDE*(0.0+metric_continuity/3.);

        /** - ----> fluid velocity */
        dy[pv->index_pt_theta_fld] = 2*a_prime_over_a*y[pv->index_pt_theta_fld]*(1. +pba->delta_DMDE/(1+w_fld)*(1.- 0.0))+k2/(1.+w_fld)*y[pv->index_pt_delta_fld];
