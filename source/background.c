...
int background_functions(
  ...
  if (pba->has_cdm == _TRUE_) {
    ...
    double w_fld_eff = pba->w0_fld+pba->delta_DMDE/3.;
    pvecback[pba->index_bg_rho_cdm] = rho_dm0*pow(a,-3.)+rho_de0*pba->delta_DMDE/(3.*w_fld_eff)*(1.-pow(a,-3.*w_fld_eff))*pow(a,-3.);
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    double rho_de0 = pba->Omega0_fld*pow(pba->H0,2.);
    double w_fld_eff = pba->w0_fld+pba->delta_DMDE/3.;
    pvecback[pba->index_bg_rho_fld] = rho_de0*pow(a,-3.*(1.+w_fld_eff));
    /* Hanyu */
